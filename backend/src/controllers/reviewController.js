...
const addReview = async (req, res) => {
  try {
    ...
    // Step 3: Prevent the uploader from reviewing their own note (only if logged in)
    if (req.user && note.uploadedBy.toString() === req.user._id.toString()) {
      return res.status(400).json({
        success: false,
    ...
    // Step 5: Populate user info only if user exists
    if (review.user) {
